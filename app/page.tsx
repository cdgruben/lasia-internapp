...
type TimeEntry = {
  ...
  work_type: WorkType;
  approved: boolean;
  orders?: { order_number: string; customer_name: string; tripletex_id: string | null; status: Status } | null;
  profiles?: { full_name: string; email: string } | null;
};

type OrderForm = ReturnType<typeof emptyOrder>;
type TimeForm = ReturnType<typeof emptyTime>;
type PlanForm = { employee_id: string; date: string; start_time: string; estimated_hours: string };

const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
const statusLabels: Record<Status, string> = {
  planning: "Til planlegging",
  scheduled: "Planlagt",
  in_progress: "Pågår",
  completed_pending_invoice: "Ferdig - til fakturering",
  invoiced_archived: "Fakturert / arkiv"
};
...
const statusClasses: Record<Status, string> = {
  planning: "border-slate-200 bg-white",
  scheduled: "border-emerald-200 bg-emerald-50",
  in_progress: "border-blue-200 bg-blue-50",
  completed_pending_invoice: "border-amber-200 bg-amber-50",
  invoiced_archived: "border-slate-200 bg-slate-50 opacity-80"
};
...
function emptyTime(orderId = "") { return { order_id: orderId, entry_date: todayIso(), start_time: "08:00", end_time: "16:00", hours: "7.5", comment: "", work_type: "ordinaer" as WorkType, mark_complete: false }; }
...
export default function App() {
  ...
  async function createOrder(e: FormEvent) {
    ...
    const { error } = await supabase.from("orders").insert({
      ...
      contact_person: orderForm.contact_person || null,
      phone: orderForm.phone || null,
      description: orderForm.description,
      assigned_employee_id: hasSchedule ? orderForm.assigned_employee_id : null,
      order_date: hasSchedule ? orderForm.order_date : null,
      scheduled_start: schedule?.startIso ?? null,
      scheduled_end: schedule?.endIso ?? null,
      estimated_hours: Number(orderForm.estimated_hours) || null,
      internal_comment: orderForm.internal_comment || null,
      status: hasSchedule ? "scheduled" : "planning"
    });
    if (error) setMessage(error.message); else { setOrderForm(emptyOrder()); await loadData(profile.id); setTab(hasSchedule ? "ordre" : "planning"); }
  }
  ...
  async function planOrder(e: FormEvent) {
    ...
    if (error) setMessage(error.message); else { setPlanningOrderId(null); setPlanForm(defaultPlanForm()); await loadData(profile.id); }
  }

  async function saveTime(e: FormEvent) {
    e.preventDefault(); if (!profile) return;
    const { error } = await supabase.from("time_entries").insert({ order_id: timeForm.order_id, employee_id: profile.id, entry_date: timeForm.entry_date, start_time: timeForm.start_time, end_time: timeForm.end_time, hours: Number(timeForm.hours) || 0, comment: timeForm.comment || null, work_type: timeForm.work_type });
...
function OrderList({ orders, profile, onAdminStatus, onStart, onPlan, onInvoice, compact = false }: { orders: Order[]; profile: Profile; onAdminStatus?: (o: Order, s: Status) => void; onStart?: (o: Order) => void; onPlan?: (o: Order) => void; onInvoice?: (o: Order) => void; compact?: boolean }) { if (!orders.length) return <p className="text-sm text-slate-600">Ingen ordre.</p>; return <div className="grid gap-2">{orders.map((o) => <article key={o.id} className={`rounded-lg border p-3 ${statusClasses[o.status]}`}><div className="flex justify-between gap-2"><div><p className="text-xs font-semibold uppercase">{o.order_number}</p><h3 className="font-semibold">{o.customer_name}</h3></div><span className="text-xs font-semibold">{statusLabels[o.status]}</span></div>{!compact && <><p className="mt-2 text-sm text-slate-700">{o.address}</p><p className="text-sm text-slate-600">{orderDay(o) ? dateLabel(orderDay(o)) : "Ikke planlagt"} · {timeLabel(o.scheduled_start)}-{timeLabel(o.scheduled_end)} · {o.estimated_hours ?? 0} t estimert</p><p className="mt-2 whitespace-pre-line text-sm">{o.description}</p></>}{profile.role === "admin" && onAdminStatus && o.status !== "planning" && o.status !== "invoiced_archived" && <select className="mt-3 w-full rounded-lg border bg-white p-2 text-sm" value={o.status} onChange={(e) => onAdminStatus(o, e.target.value as Status)}>{Object.entries(statusLabels).filter(([k]) => k !== "planning" && k !== "invoiced_archived").map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>}{profile.role === "admin" && onPlan && <button onClick={() => onPlan(o)} className="mt-3 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white">Planlegg</button>}{profile.role === "admin" && onInvoice && <button onClick={() => onInvoice(o)} className="mt-3 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white">Marker som fakturert</button>}{profile.role === "employee" && o.status === "scheduled" && onStart && <button onClick={() => onStart(o)} className="mt-3 rounded-lg border border-emerald-700 px-3 py-2 text-sm font-semibold text-emerald-700">Sett pågår</button>}</article>)}</div>; }
